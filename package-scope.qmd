...
This section details the Epiverse-TRACE reasoning for the inclusion or exclusion of functionality around plotting. It is not a commentary generalisation to all R packages. For some packages it is obvious that plotting is superfluous or nonsensical, whereas for others, the purpose of the package may be plotting or the package could be a graphics library (e.g., {ggplot2}).
...
A benefit of including plotting functions is it enables package users to quickly inspect a feature of the package (e.g. model output) without having to write their own scripts or functions. Despite this clear benefit of shipping plotting with a package there are several
...
order for an R package to export plotting functions that use {ggplot2}, it must list {ggplot2}
as a [dependency](https://cran.r-project.org/doc/manuals/R-exts.html#Package-Dependencies). {ggplot2} is a "heavy" package, not so much from the size of the code base (as 
CRAN limits the size of packages to 5MB), but due to it's own dependencies.
