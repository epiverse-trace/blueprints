...
In particular, packages that are not available on CRAN or Bioconductor should not be included as dependencies. Only CRAN/Bioconductor packages that already went through a couple of releases and/or have more than 100 monthly users can be considered as dependencies.
This policy applies for taking both Epiverse-TRACE packages or external packages as dependencies.
It aims at ensuring the robustness of the ecosystem we are building but also allowing maturing packages to go through necessary breaking changes unencumbered.
