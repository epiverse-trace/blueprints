...
---
title: "Software development blueprint for _Epiverse-TRACE_"
output:
...
```{r cc-by}
 ...
 {
   knitr::include_graphics("https://github.com/epiverse-trace/blueprints/raw/main/figures/cc-by-logo.png")
 } else {
   knitr::include_url("https://github.com/epiverse-trace/blueprints/raw/main/figures/cc-by-logo.png")
 }
...
```{r mindmap}
 ...
 if (knitr::is_html_output())
 {
   knitr::include_graphics("https://raw.githubusercontent.com/epiverse-trace/blueprints/main/figures/mindmap.png")
 } else {
   knitr::include_url("https://raw.githubusercontent.com/epiverse-trace/blueprints/main/figures/mindmap.png")
 }
...
Whether we reuse entire source files or libraries, or we paste source code from third-party authors, we must properly credit them and ensure any license allows us to properly reuse their code.

* When directly inserting/adapting external code into our source files, add a comment right before said code using the following format:
  ```
  // credits: SOURCE_URL
  ```
  Where `SOURCE_URL` is the url where we obtained the source code (_e.g._ stackoverflow, etc.). That way we may be able to automatically create a document that summarizes such credits
* When reusing entire libraries, ensure that their licenses are compatible with ours. For instance, if we chose MIT or a similar license for our projects, it may not be a good idea to reuse GPL code, since we would be compelled to change our license to GPL, too. In general, we should would avoid any library with licenses --either open or non non open source-- that are more restrictive than ours
